...
def test_sanitize_server_name_valid(raw: str) -> None:
    """Valid server names are trimmed and returned unchanged."""
    assert sanitize_server_name(raw) == raw.strip()
...
def test_sanitize_server_name_invalid_raises(raw: str) -> None:
    """Invalid server names raise ``AuthorizationError``."""
    with pytest.raises(AuthorizationError):
        sanitize_server_name(raw)
